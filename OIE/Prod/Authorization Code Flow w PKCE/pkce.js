...
async function generateAuthURI() {
  ...
  const authURI = `https://jessica-prod.com/oauth2/${process.env.OKTA_CUSTOM_AUTH_SERVER}/v1/authorize?client_id=${process.env.CLIENT_ID}&response_type=code&scope=profile openid offline_access&redirect_uri=http://localhost:8080/login/callback&state=${process.env.OKTA_CUSTOM_AUTH_SERVER}&code_challenge_method=S256&code_challenge=${challenge}`;
  const pkceUtil = {
    authURI: authURI,
    verifier: verifier,
    challenge: challenge
